...
过滤器用在两个地方：插值表达式和v-bind表达式。接收|符号前的内容作为第一个参数。
...
此处要注意：定义时不需要加v-前缀，调用时需要加上v-前缀。私有优先全局。

#### 钩子函数

一个指令定义对象可选钩子函数：bind、inserted、updated、componentUpdated、unbind。
...
- value：指令绑定的值。例如v-my="1+1"中，为2。

- oldValue：指令绑定的前一个值，仅在update和 componentUpdated钩子中可用。无论值是否改变都可用。

- expression：字符串形式的指令表达式。例如v-my="1+1"中，为1+1。

- arg：传给指令的参数。例如v-my:foo中，为foo

- modifiers：一个包含修饰符的对象。例如v-my.foo.bar中，修饰符对象为{ foo : true, bar : true }。
...
如果指令需要多个值，可以传入一个JavaScript对象字面量。指令函数能够接受所有合法的 JavaScript 表达式。
